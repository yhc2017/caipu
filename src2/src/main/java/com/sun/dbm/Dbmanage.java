...
public class Dbmanage {
     ...
     */
    public static ConnectionParam connectionParam=new ConnectionParam();
    public static ConnectionPool connectionPool=new ConnectionPool();
    public static Connection con= null;
    static{
        connectionParam.setDriver("com.mysql.jdbc.Driver");

//        connectionParam.setUrl("jdbc:mysql://localhost/recipe");
//        connectionParam.setUser("root");
//        connectionParam.setPassword("root");

        connectionParam.setUrl("jdbc:mysql://localhost/recipe"); //recipe 为数据库名
        connectionParam.setUser("root"); //用户名
        connectionParam.setPassword("0718"); //用户密码
...
//    }
    ...
    public void closeDBWithoutres(Statement sta, Connection conn) {
        // 关闭数据库连接（无结果集）
        try {
            if (sta != null) {
                sta.close();
